...
$('.grid-expand').on('click', function () {
    if ($(this).data('inserted') == '0') {
        $('.expand').remove();
        $('.grid-expand').data('inserted', 0);
        $(".expand-i").attr("class","expand-i fa fa-caret-right");
        var key = $(this).data('key');
        var row = $(this).closest('tr');
        var html = $('template.grid-expand-'+key).html();

        row.after("<tr class='expand'><td colspan='"+row.find('td').length+"' style='padding:0 !important; border:1px !important;'>"+html+"</td></tr>");
...
<a class="btn btn-xs btn-default grid-expand" data-inserted="0" data-key="{$key}" data-toggle="collapse" data-target="#grid-collapse-{$key}">
    <i class="expand-i fa fa-caret-right"></i> $btn
</a>
